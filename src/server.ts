 ...
 *
 * Exposes Card Forge Cards, Stacks, Decks, and Receipts to MCP-compatible
 * agents. Local/default mode speaks MCP over stdio. Fly/HTTP mode exposes a
 * Streamable HTTP endpoint at /mcp plus a health check at /healthz.
 *
 * v0.1 boundary (see CLAIM_BOUNDARY.md):
 *   - read-only: no write/mutate tools
 *   - no arbitrary shell execution
 *
 * IMPORTANT: stdio mode uses stdout as the MCP transport. All human-readable
 * logging goes to stderr via console.error. Never console.log here.
 */
...
function createCardsAndDecksServer(store: Store) {
  ...
  server.tool(
    "get_stack",
    "Get one Stack by id, with resolved Cards.",
    { stack_id: z.string() },
    async ({ stack_id }) => {
...
async function loadStoreForStartup(store: Store) {
...
async function startStdio() {
  ...
  await loadStoreForStartup(store);

  const server = createCardsAndDecksServer(store);
...
async function startHttp() {
  ...
  const mcpServer = createCardsAndDecksServer(store);
  ...
  const httpServer = createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === "/healthz") {
        sendJson(res, 200, { ok: true, service: "cards-and-decks-mcp", transport: "streamable-http" });
        return;
      }

      if (url.pathname !== "/mcp") {
        sendJson(res, 404, { error: "not_found", message: "Use /mcp for MCP or /healthz for health." });
        return;
      }
      ...
      console.error(`[cards-and-decks-mcp] HTTP fatal: ${(err as Error).stack ?? err}`);
      if (!res.headersSent) {
  ...
  httpServer.listen(port, host, () => {
    console.error(`[cards-and-decks-mcp] ready on http://${host}:${port}/mcp. read-only v0.1.`);
  });

  const shutdown = async () => {
    console.error("[cards-and-decks-mcp] shutting down HTTP transport.");
